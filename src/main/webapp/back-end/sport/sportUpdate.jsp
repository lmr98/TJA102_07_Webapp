...
<head>
<meta charset="UTF-8">
<title>[更新] 系統運動資料- sportUpdate.jsp</title>
...
<body bgcolor='white'>
	...
	<div class="uni-subject-block">
		...
		<div class="uni-subject-block-l">
			<div class="uni-subject-text">[更新] 系統運動資料</div>
			<div>~ sportUpdate.jsp ~</div>
			<a class="uni-turn-a"
	...
	<div class="uni-form-block">
		...
		<hr />
		<div class="uni-subject-text">請更新下列資料</div>
		<hr />

		<jsp:useBean id="sportSvc" scope="page"
			class="com.tibafit.sport.model.SportService" />
		<FORM METHOD="post"
			ACTION="<%=request.getContextPath()%>/sport/sport_do" name="form1">
			<table>
				<tr>
					<td><span class="required-text">*</span>運動編號:</td>
					<td><input type="hidden" name="sportId"
						value="<%=(sportVO == null || sportVO.getSportId() == null) ? null : sportVO.getSportId()%>" />
						<span><%=(sportVO == null || sportVO.getSportId() == null) ? "" : sportVO.getSportId()%></span>
					</td>
				</tr>
				<tr>
				...
				<tr>
					<td><span class="required-text">*</span>估算消耗熱量 <br>
						(60kg，1hr):</td>
				...
				<tr>
					<td><span class="required-text">*</span>運動資料狀態:</td>
					<td><select size="1" name="sportDataStatus">
							<c:forEach var="sportDataStatus"
								items="${sportSvc.sportDataStatusOptions}">
								<option value="${sportDataStatus.codeNum}"
									${sportDataStatus.codeNum == sportVO.getSportDataStatus()? 'selected' : ''}>
									${sportDataStatus.displayName}</option>
							</c:forEach>
					</select></td>
				<tr>
			</table>
			<br> <input type="hidden" name="action" value="update">
			<input class="uni-send-btn" type="submit" value="送出更新">
		</FORM>
	</div>
