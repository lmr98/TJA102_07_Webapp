...
public class SportDAO implements SportDAO_Interface {
	...
	static {
		try {
			...
			ds = (DataSource) ctx.lookup("java:comp/env/jdbc/TJA102_G3_TibaFit_SJver");
	...
	public void updateSport(SportVO sportVO) {
		...
		try {
			...
			if (sportVO.getSportId() == null) {
				throw new IllegalArgumentException("getSportId return null");
			}
			...
			throw new RuntimeException("SportDAO updateSport Error : " + e.getMessage());
		} catch (SQLException e) {
...
class SportDynamicParam {
	Object inputValue;
	int sqlType;

	public SportDynamicParam(Object value, int sqlType) {
		super();
...
class SportResultOneException extends Exception{
    public SportResultOneException(String message) {
        super(message);
    }
}